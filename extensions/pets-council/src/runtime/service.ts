...
export class CodexRuntimeService {
  ...
  setResumeCandidate(candidate: CodexResumeCandidate | undefined): void {
    if (
      candidate?.threadId === this.statusValue.resumeCandidate?.threadId
      && candidate?.savedAt === this.statusValue.resumeCandidate?.savedAt
    ) {
      return;
    }
  ...
  setBinary(binary: string): void {
    ...
    if (value === this.statusValue.binary) {
      return;
    }
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.set({
        phase: 'ready',
        binary,
        message: 'Connected. Choose whether to start a new session or resume the saved one.',
  ...
  async startThread(cwd?: string): Promise<void> {
    ...
    if (this.statusValue.thread.phase === 'starting' || running(this.statusValue.turn)) {
      return;
    }
    ...
    this.set({
      ...this.statusValue,
      thread: { phase: 'starting', message: 'Creating a new Codex thread for this workspace…' },
      turn: idleTurn(),
      approval: undefined
    });
    ...
    try {
      ...
      if (!this.isCurrentThreadOperation(client, sequence)) {
        return;
      }
      const resumeCandidate = { threadId: thread.id, savedAt: Date.now() };
      this.set({
        ...this.statusValue,
        thread: {
          phase: 'ready',
          message: 'New thread ready. No turn has been started and no prompt has been sent.',
          thread
        },
        turn: idleTurn(),
        approval: undefined,
      ...
      this.setThread({ phase: 'error', message: normalize(error) });
    }
  }
  ...
  async resumeThread(threadId?: string, cwd?: string): Promise<void> {
    ...
    if (!candidate) {
      this.setThread({ phase: 'error', message: 'No saved Codex session is available for this workspace.' });
      return;
    }
    ...
    this.set({
      ...this.statusValue,
      thread: { phase: 'starting', message: 'Resuming the saved Codex thread…' },
      turn: idleTurn(),
      approval: undefined
    });
    ...
    try {
      ...
      this.set({
        ...
        thread: {
          phase: 'ready',
          message: restored
            ? 'Saved thread resumed with its last completed exchange.'
            : 'Saved thread resumed. No completed text exchange was available to restore.',
          thread
        },
        turn: restored
          ? {
            ...
            userMessage: restored.userMessage,
        ...
        resumeCandidate: {
          threadId: thread.id,
          savedAt: this.statusValue.resumeCandidate?.savedAt ?? Date.now()
        }
      });
    } catch (error) {
      if (sequence !== this.threadSequence || this.client !== client) {
  ...
  async startTurn(userMessage: string, cwd?: string): Promise<void> {
    ...
    if (!text) {
      this.setTurn({ phase: 'error', message: 'Write a prompt before starting a Codex turn.' });
      return;
    }
    ...
    if (!client || this.statusValue.thread.phase !== 'ready' || !thread) {
      this.setTurn({
        phase: 'error',
        message: 'Start or resume a Codex session before sending a prompt.',
        userMessage: text
      });
      return;
    }
    if (running(this.statusValue.turn) || this.statusValue.approval) {
      return;
    }
    ...
    try {
      ...
      if (sequence !== this.turnSequence || this.client !== client || this.statusValue.phase !== 'ready') {
        return;
      }
      const current = this.statusValue.turn;
      if (current.turnId === turnId && (current.phase === 'completed' || current.phase === 'error')) {
        return;
      }
      this.setTurn({
        ...current,
  ...
  async interruptTurn(): Promise<void> {
    ...
    const threadId = this.statusValue.thread.thread?.id;
    const turnId = this.statusValue.turn.turnId;
    if (!client || !threadId || !turnId || !running(this.statusValue.turn)) {
      return;
    }
    if (this.statusValue.approval) {
      this.respondApproval('decline');
    }
    this.setTurn({ ...this.statusValue.turn, message: 'Interrupting the Codex turn…' });
    try {
      await client.interruptTurn(threadId, turnId);
    } catch (error) {
      this.setTurn({ ...this.statusValue.turn, phase: 'error', message: normalize(error) });
    }
  }

  respondApproval(decision: CodexApprovalDecision): void {
    const approval = this.statusValue.approval;
    if (!approval || !this.client) {
      return;
    }
    ...
    this.set({
      ...
      turn: {
        ...
        message: decision === 'accept'
          ...
          : 'Approval declined. Waiting for Codex…'
      }
    });
  }
  ...
  disconnect(): void {
    ...
    const resumeCandidate = this.statusValue.resumeCandidate;
    this.replaceClient(undefined);
    this.set({ ...disconnected(binary), resumeCandidate });
  }
  ...
  private readyClient(): CodexAppServerClient | undefined {
    if (this.statusValue.phase !== 'ready' || !this.client) {
      this.setThread({ phase: 'error', message: 'Connect Codex before choosing a session.' });
      return undefined;
    }
    return this.client;
  }

  private isCurrentThreadOperation(client: CodexAppServerClient, sequence: number): boolean {
    return sequence === this.threadSequence
      && this.client === client
  ...
  private replaceClient(client: CodexAppServerClient | undefined): void {
    ...
    this.closeSub = this.notificationSub = this.requestSub = undefined;
    ...
    this.closeSub = client.onDidClose((reason) => {
    ...
    this.notificationSub = client.onNotification((value) => {
      if (this.client === client) {
        this.handleNotification(value);
      }
    });
    this.requestSub = client.onRequest((value) => {
      if (this.client === client) {
  ...
  private handleRequest(request: JsonRpcServerRequest): void {
    ...
    const approval = params ? parseApproval(request, params) : undefined;
    if (!approval) {
      return;
    }
    const activeThread = this.statusValue.thread.thread?.id;
    const activeTurn = this.statusValue.turn.turnId;
    if (
      (activeThread && approval.threadId !== activeThread)
      || (activeTurn && approval.turnId !== activeTurn)
    ) {
      return;
    }
    this.set({
      ...this.statusValue,
      approval,
      turn: { ...this.statusValue.turn, message: 'Codex is waiting for your approval.' }
    });
  }
  ...
  private handleNotification(notification: JsonRpcNotification): void {
    ...
    const activeThread = this.statusValue.thread.thread?.id;
    const notificationThread = optionalString(params.threadId);
    if (activeThread && notificationThread && notificationThread !== activeThread) {
      return;
    }
    ...
    if (notification.method === 'item/agentMessage/delta') {
      ...
      if (!delta || !matches(this.statusValue.turn, turnId)) {
        return;
      }
      this.setTurn({
        ...this.statusValue.turn,
    ...
    if (notification.method === 'item/completed') {
      const turnId = optionalString(params.turnId);
      if (!matches(this.statusValue.turn, turnId)) {
        return;
      }
    ...
    if (notification.method === 'turn/completed') {
      ...
      this.set({
        ...
        turn: {
          ...
          message: failed
            ? optionalString(objectValue(turn?.error)?.message) ?? 'The Codex turn failed.'
  ...
  private setThread(thread: CodexThreadStatus): void {
    this.set({ ...this.statusValue, thread });
  }

  private setTurn(turn: CodexTurnStatus): void {
    this.set({ ...this.statusValue, turn });
  }

  private set(status: CodexRuntimeStatus): void {
...
function parseApproval(
  ...
  return {
    ...
    kind: request.method.includes('commandExecution') ? 'commandExecution' : 'fileChange',
    threadId,
    turnId,
    itemId,
    ...
    startedAtMs: numberValue(params.startedAtMs)
  };
}
...
export function resolveCodexBinary(
  ...
  return configured?.trim() || environment.CODEX_BIN?.trim() || 'codex';
}

function disconnected(binary: string): CodexRuntimeStatus {
  return {
    phase: 'disconnected',
    binary,
...
function noThread(): CodexThreadStatus {
  return { phase: 'none', message: 'No Codex thread exists for this runtime connection.' };
}

function idleTurn(): CodexTurnStatus {
  return { phase: 'idle', message: 'No Codex turn has been started in this session.' };
}

function running(turn: CodexTurnStatus): boolean {
  return turn.phase === 'starting' || turn.phase === 'streaming';
}

function matches(turn: CodexTurnStatus, id: string | undefined): boolean {
  return !turn.turnId || !id || turn.turnId === id;
}
