...
class FakeTransport implements CodexMessageTransport {
  ...
  send(message: unknown): void {
    ...
    if (request.method === 'model/list') this.reply({id:request.id,result:{data:[{id:'gpt-test',model:'gpt-test',displayName:'GPT Test',isDefault:true,supportedReasoningEfforts:['medium'],defaultReasoningEffort:'medium'}]}});
    if (request.method === 'config/read') this.reply({id:request.id,result:{model:'gpt-test',model_reasoning_effort:'medium'}});
