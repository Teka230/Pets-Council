import type { CodexInitializeResult, CodexMessageTransport, CodexRestoredTurn, CodexThreadInfo, CodexTransportFactory, JsonRpcId, JsonRpcNotification, JsonRpcResponse, JsonRpcServerRequest, RuntimeDisposable } from './types';
...
export class CodexAppServerClient {
  ...
  private constructor(private readonly transport:CodexMessageTransport,readonly serverInfo:CodexInitializeResult){this.messageSub=transport.onMessage(message=>this.handle(message));this.closeSub=transport.onClose(reason=>this.close(reason,false));}
  static async connect(factory:CodexTransportFactory,binary:string,timeoutMs=TIMEOUT):Promise<CodexAppServerClient>{const transport=await factory(binary),bootstrap=new Bootstrap(transport);try{const info=await bootstrap.initialize(timeoutMs);bootstrap.detach();return new CodexAppServerClient(transport,info);}catch(error){bootstrap.dispose();throw error;}}
  request<T=unknown>(method:string,params?:unknown,timeoutMs=TIMEOUT):Promise<T>{if(this.closed)return Promise.reject(new Error('Codex app-server connection is closed.'));const id=this.nextId++;return new Promise<T>((resolve,reject)=>{const timeout=setTimeout(()=>{this.pending.delete(id);reject(new Error(`Codex app-server request timed out: ${method}`));},timeoutMs);this.pending.set(id,{resolve:value=>resolve(value as T),reject,timeout});this.transport.send({id,method,...(params===undefined?{}:{params})});});}
  async startThread(cwd?:string):Promise<CodexThreadInfo>{return parseThread(await this.request('thread/start',{...(cwd?{cwd}:{}),approvalsReviewer:'user'}));}
  async resumeThread(threadId:string,cwd?:string):Promise<CodexThreadInfo>{return parseThread(await this.request('thread/resume',{threadId,...(cwd?{cwd}:{}),approvalsReviewer:'user'}));}
  async startTurn(threadId:string,text:string,cwd?:string):Promise<string>{return parseTurnId(await this.request('turn/start',{threadId,input:[{type:'text',text}],...(cwd?{cwd}:{}),approvalsReviewer:'user'}));}
  ...
  private handle(message:unknown):void{if(isServerRequest(message)){for(const listener of this.requests)listener(message);return;}if(isResponse(message)){const pending=this.pending.get(message.id);if(!pending)return;this.pending.delete(message.id);clearTimeout(pending.timeout);if(message.error)pending.reject(new Error(`Codex app-server error ${message.error.code}: ${message.error.message}`));else pending.resolve(message.result);return;}if(isNotification(message))for(const listener of this.notifications)listener(message);}
...
class Bootstrap{
  ...
  async initialize(timeoutMs:number):Promise<CodexInitializeResult>{const result=await this.request('initialize',{clientInfo:{name:'pets_council',title:'Pets Council',version:'0.9.0'},capabilities:{}},timeoutMs);const info=parseInitialize(result);this.transport.send({method:'initialized'});return info;}
