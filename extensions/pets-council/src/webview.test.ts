...
function render(turn=SAMPLE_COUNCIL_TURN,status=runtime,turns:readonly CodexRestoredTurn[]=history,motion:'system'|'full'|'reduced'='system'):string{const review=reviewMockTurn(turn);return renderCouncilHtml(turn,review,council,status,buildPetSnapshots(review,council,status),turns,motion,'nonce');}
