...
export function activate(context:vscode.ExtensionContext):void{
  ...
  const openCouncil=():void=>{showCouncilPanel(runtime,graphStore,usageStore,nativeOverlay);void offerProductLayout(context);};
  ...
  const commands=[
    vscode.commands.registerCommand('petsCouncil.openCouncil',openCouncil),
    vscode.commands.registerCommand('petsCouncil.applyProductLayout',()=>applyProductLayout(context,openCouncil)),
...
function showCouncilPanel(runtime:CodexRuntimeService,graphStore:SharedContextGraphStore,usageStore:SuggestionUsageSignalStore,nativeOverlay:NativeOverlayBridge):void{
  ...
  const reviewCompletedTurn=async(turn:CouncilTurn):Promise<void>=>{
    ...
    reviewTimeline=upsertCouncilTurnReview(reviewTimeline,{turn,review:currentReview,state:councilState});renderCurrent();
    ...
    currentReview=outcome.review;councilState=outcome.state;reviewTimeline=upsertCouncilTurnReview(reviewTimeline,{turn,review:outcome.review,state:outcome.state});renderCurrent();
  };
  ...
  const messageSubscription=panel.webview.onDidReceiveMessage(async(message:unknown)=>{
    ...
    switch(message.type){
      case'refreshContext':await refreshContext();return;case'openFolder':await openFolderFromCouncil();return;case'connectCodex':await runtime.connect();return;case'disconnectCodex':runtime.disconnect();return;
      case'startCodexThread':await runtime.startThread(currentWorkspaceDirectory());return;case'resumeCodexThread':await runtime.resumeThread(undefined,currentWorkspaceDirectory());return;case'startCodexTurn':await runtime.startTurn(message.value,currentWorkspaceDirectory());return;case'interruptCodexTurn':await runtime.interruptTurn();return;
      case'setCodexModel':runtime.setModelSelection({model:message.model,modelProvider:message.modelProvider});return;case'setCodexEffort':runtime.setModelSelection({effort:message.effort});return;case'respondCodexApproval':runtime.respondApproval(message.decision);return;
      case'saveCouncilSuggestion':await saveSuggestion(message.suggestionId);return;case'recordSuggestionSignal':await recordUsage(message.suggestionId,message.action);return;case'openContextGraph':await graphStore.open();return;case'openUsageSignals':await usageStore.open();return;
      case'browseCodexSessions':await vscode.commands.executeCommand('petsCouncil.browseCodexSessions');return;case'applyProductLayout':await vscode.commands.executeCommand('petsCouncil.applyProductLayout');return;
